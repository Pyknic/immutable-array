...
public interface LongImmutableArray {
     ...
     */
    long getAsLong(long index);
    
    /**
     * Returns the length of the array (the number of longs that can safely
     * be retrieved using the {@link #getAsLong(long)} method).
     * 
     * @return  the length
     */
    long length();
    ...
    /**
     * A builder for a {@link LongImmutableArray}. The builder can be appended
     * with values using the {@link #append(long)}-method until it is finalized
